...
function mrts_val = calculate_auto_mrts(spikes_trains)
    % Automatics calcul of MRTS parameter (Means of the smallest ISI of the dataset)
    min_isis = [];
    for i = 1:length(spikes_trains)
        if length(spikes_trains{i}) > 1
            min_isis = [min_isis, min(diff(spikes_trains{i}))];
        end
    end
    if isempty(min_isis)
        mrts_val = 0; % Valeur de secours
    else
        mrts_val = mean(min_isis);
    end
end
